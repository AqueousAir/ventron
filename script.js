(() => {
  ...
  const contactForm = document.querySelector('#contact-simple-form');
  if (contactForm) {
    const status = contactForm.querySelector('.form-status');
    const submitButton = contactForm.querySelector('button[type="submit"]');
    const successMessage = 'Thank you. Your inquiry has been sent to VENTRON ENERGY. Our team will review it and contact you shortly.';
    const errorMessage = 'We could not send the inquiry automatically. Please try again shortly.';
    ...
    const contactFormDataToObject = formData => {
      const data = {};
      formData.forEach((value, key) => {
        if (key.startsWith('_')) return;
        data[key] = value;
    ...
    const buildContactPayload = formData => {
      const data = contactFormDataToObject(formData);
      const lines = [
        'New VENTRON ENERGY Project Inquiry',
        '',
        'Contact Information',
        '-------------------',
        `Full Name: ${data['Full Name'] || 'Not provided'}`,
        `Email Address: ${data['Email Address'] || 'Not provided'}`,
        '',
        'Project Information',
        '-------------------',
        `Product Interest: ${data['Product Interest'] || 'Not provided'}`,
        `Project Location: ${data['Project Location'] || 'Not provided'}`,
        '',
        'Message:',
      ...
      return {
        subject: formData.get('_subject') || 'New VENTRON ENERGY Project Inquiry',
        page: window.location.href,
        submittedAt: new Date().toISOString(),
        fields: data,
        emailBody: lines.join('\n')
    ...
    contactForm.addEventListener('submit', async event => {
      ...
      if (!contactForm.checkValidity()) {
        ...
        setContactStatus('Please complete the required fields before submitting.', 'error');
        return;
      }
      ...
      if (!endpoint || endpoint.includes('PASTE_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE')) {
        setContactStatus('Google Apps Script endpoint is not connected yet. Paste the deployed Web App URL into the form data-endpoint attribute.', 'error');
        return;
      }
      ...
      submitButton?.setAttribute('disabled', 'disabled');
      setContactStatus('Submitting your inquiry...', '');
      ...
      try {
        ...
        contactForm.reset();
        setContactStatus(successMessage, 'success');
      } catch (error) {
